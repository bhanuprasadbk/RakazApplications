...
module.exports = {
    ...
    getCustomerMappedToSensors: async (req, res) => {
        try {
            const { sensor_types } = req.body; // Get sensorType from request body
            console.log(sensor_types);
            if (!sensor_types || !Array.isArray(sensor_types)) {
                return res.status(400).json({
                    success: false,
                    message: 'sensorType array is required in request body'
                });
            }
            if (sensor_types.length === 0) {
                successlog.info('Empty sensor_types array provided, returning empty results');
                return res.status(200).json({
                    success: true,
                    message: 'Customer mapped to sensors fetched successfully',
                    data: []
                });
            }
            customersModel.getCustomerMappedToSensors(sensor_types, (error, results) => {
                if (error) {
                    errorlog.error('Error fetching customer mapped to sensors:', error);
                    return res.status(500).json({
                        success: false,
                        message: 'Error fetching customer mapped to sensors',
                        error: error.message
                    });
                }
                successlog.info(`Customer mapped to sensors fetched for sensor types: ${sensor_types.join(', ')}`);
                return res.status(200).json({
                    success: true,
                    message: 'Customer mapped to sensors fetched successfully',
                    data: results
            ...
            errorlog.error('Exception in getCustomerMappedToSensors:', error);
            return res.status(500).json({
                success: false,
                message: 'Internal server error',
                error: error.message
